...
def calcTPscoreOld(ys,flashseq,target,isSpeller=1,doImg=0):
...
def calcTPscore(ys, flashseq, target, plot=False):
    ...
    for subtrial_i in range(subtrials_number):
        ...
        for epoch_i in range(row_col_number):
        ...
        if plot:
            M_scaled = scale(M_vec[subtrial_i].ravel(), 0, 1).reshape(rows_number, cols_number)
    ...
    if subtrial_index is not None:
        ...
        M_thr = scale(M_vec[subtrial_index].ravel(), 0, 1).sum()
    else:
        TPscore  = 0
        M_thr = 0
...
def fda_train(data, label):
    ...
    invSw = linalg.inv(Sw)
    #changed from fda_w = invSw @ (posmean-negmean).T
